...
int CAutowall::CalculateDamage(C_CSPlayerPawn* pTargetPawn, C_CSWeaponBaseGun* pBaseGun, Vector vecEnd) {
	if (!pTargetPawn || !Globals::m_pLocalPlayerPawn) return 0;

	penetration_data_t pen_data{};
	Vector eye_pos = Globals::m_pLocalPlayerPawn->GetEyePosition(true);

	bool can_hit = g_Autowall->m_Functions.m_FireBullet.FireBullet(eye_pos, vecEnd, &pen_data, pTargetPawn);
	if (!can_hit || pen_data.m_damage < 1.0f) return 0;

	return static_cast<int>(pen_data.m_damage);
}
...
float CAutowall::CFunctions::CScaleDamage::ScaleDamage(
	C_CSPlayerPawn* pPawn,
	CCSWeaponBaseVData* pWeaponVData,
	int iHitGroup,
	float& flDamage)
{
	if (!pPawn || !pWeaponVData)
		return flDamage;
	...
	bool hasHelmet = pItemServices->m_bHasHelmet();

	// Team-based head/body scale (mp_damage_scale convarl)
	float flHeadScale = 1.0f, flBodyScale = 1.0f;
	switch (pPawn->GetAssociatedTeam()) {
	case TEAM_TT:
		flHeadScale = Convar::mp_damage_scale_t_head->GetFloat();
		flBodyScale = Convar::mp_damage_scale_t_body->GetFloat();
		break;
	case TEAM_CT:
		flHeadScale = Convar::mp_damage_scale_ct_head->GetFloat();
		flBodyScale = Convar::mp_damage_scale_ct_body->GetFloat();
		break;
	}
	...
	switch (iHitGroup) {
	case HITGROUP_HEAD:
		flDamage *= pWeaponVData->m_flHeadshotMultiplier() * 4.0f * flHeadScale; 
		break;
	case HITGROUP_CHEST:
	...
	if (!pPawn->IsArmored(iHitGroup))
		return std::max(1.0f, flDamage);

	float flArmor = (float)pPawn->m_ArmorValue();
	float armor_ratio = pWeaponVData->m_flArmorRatio() * 0.5f;   
	float armor_bonus = 0.5f;

	float dmg_to_armor = flDamage * (1.0f - armor_ratio) * armor_bonus;

	if (dmg_to_armor > flArmor) {
		float excess = dmg_to_armor - flArmor;
		flDamage = (flDamage * armor_ratio) + (excess / armor_bonus);
	}
	else {
		flDamage = (flDamage * armor_ratio) + (dmg_to_armor / armor_bonus);
	}

	if (iHitGroup == HITGROUP_HEAD && hasHelmet) {
		flDamage *= 0.85f; // Valve helmet reduction ~15%
	}

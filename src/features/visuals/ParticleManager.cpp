    ...
    constexpr const char* SNOW_EFFECT_PATH = "particles/rain_fx/snow.vpcf";
    ...
    void CParticleMgr::OnEvent(CGameEvent* pEvent)
    {
        ...
        if (pController->m_bIsLocalPlayerController())
        {
            ...
            {
                if (!Interfaces::m_pClient || !Interfaces::m_pClient->GetDebugOverlay())
                {
                    Logging::Print(X("failed to get debug overlay"));
                    return;
                }

                auto debugOverlay = Interfaces::m_pClient->GetDebugOverlay();

                debugOverlay->RenderWithoutDots(true);
    ...
    void CParticleMgr::UpdateParticles()
    {
        ...
        // spawn new particle(s) if none exist
        if (m_vecEffectIndexes.empty()) {
            switch (iSelectedEffect) {
            case WORLD_PARTICLES_SNOW:
                this->CreateParticle(m_vecEffectIndexes.emplace_back(), SNOW_EFFECT_PATH);
                break;
            case WORLD_PARTICLES_ASHES:
                this->CreateParticle(m_vecEffectIndexes.emplace_back(), EMBER_EFFECT_PATH);
                this->CreateParticle(m_vecEffectIndexes.emplace_back(), EMBER_EFFECT2_PATH);
                break;
            case WORLD_PARTICLES_STARS:
                this->CreateParticle(m_vecEffectIndexes.emplace_back(), STARS_EFFECT_PATH);
                break;
            }
        }

        // set CP0 for all active effects
        Vector vOrigin = Globals::m_pLocalPlayerPawn->m_pGameSceneNode()->m_vecAbsOrigin();
        vOrigin.z += 1200.f; // above player

        for (auto& uEffectIndex : m_vecEffectIndexes) {
            if (!uEffectIndex) continue;

            // CP0 = origin
            Interfaces::m_pParticleManager->SetParticleSetting(uEffectIndex, static_cast<EParticleSetting>(0), &vOrigin);
        }
    }

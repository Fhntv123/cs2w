...
public:
	auto Destruct() { 
		return Memory::CallVFunc<void, 1>(this, true); 
	}
...
public:
	C_EconItemDefinition* get_static_data() {
		return Memory::CallVFunc<C_EconItemDefinition*, 13>(this);
	}
	...
	bool is_agent() {
		static auto type_custom_player = FNV1A::HashConst("#Type_CustomPlayer");
		auto hash_name = FNV1A::Hash(m_pszItemTypeName);
		if (FNV1A::Hash(m_pszItemTypeName) != type_custom_player)
			return false;
		return true;
	...
	bool is_knife(bool exclude_default) {
		static auto csgo_type_knife = FNV1A::HashConst("#CSGO_Type_Knife");
	...
	bool is_glove(bool exclude_default) {
		static auto type_hands = FNV1A::HashConst("#Type_Hands");
	...
	bool is_weapon_case() {
		static auto type_weapon_case = FNV1A::HashConst("#CSGO_Type_WeaponCase");
		return FNV1A::Hash(m_pszItemTypeName) == type_weapon_case;
	}

	bool is_key() {
		static auto tool_weapon_case_key_tag = FNV1A::HashConst("#CSGO_Tool_WeaponCase_KeyTag");
		return FNV1A::Hash(m_pszItemTypeName) == tool_weapon_case_key_tag;
	}

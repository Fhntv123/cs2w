...
public:
	...
	void Init(float ix = 0.0f, float iy = 0.0f, float iz = 0.0f) { this->x = ix; this->y = iy; this->z = iz; }
	...
	[[nodiscard]] constexpr float DotProduct(const Vector& vecDot, const bool bAdditional = false) const
	{
		if (bAdditional)
	...
	[[nodiscard]] QAngle ToAngles(const Vector& vUp) const;
	[[nodiscard]] QAngle ToAngles() const;
	QAngle UpToAngles(const Vector& vUp);
...
public:
	...
	Vector4D Conjugate() const { return Vector4D(-x, -y, -z, w); }
	...
	Vector RotateVector(const Vector& v) const {
		Vector4D vQuaternion(v.x, v.y, v.z, 0.0f);
		Vector4D result = *this * vQuaternion * Conjugate();

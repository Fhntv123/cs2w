...
[[nodiscard]] QAngle Vector::ToAngles(const Vector& vUp) const {
	...
	if (flForwardDistance > 0.001f) {
		...
		return {
			atan2f(-this->z, flForwardDistance) * 180 / M_PI,
			atan2f(this->y, this->x) * 180 / M_PI,
			atan2f(vLeft.z, upZ) * 180 / M_PI
		...
		return {
			atan2f(-this->z, flForwardDistance) * 180 / M_PI,
			atan2f(-vLeft.x, vLeft.y) * 180 / M_PI,
			0.0f
		};
	}
}

QAngle Vector::UpToAngles(const Vector& vUp) {
	Vector vLeft = CrossProduct(vUp);
	vLeft.NormalizeInPlace();

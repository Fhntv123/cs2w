...
void CSHA1::Transform(UINT_32* pState, const UINT_8* pBuffer)
{
	...
	CRT::MemoryCopy(m_block, pBuffer, 64);
...
void CSHA1::Update(const UINT_8* pbData, UINT_32 uLen)
{
	...
	{
		i = 64 - j;
		CRT::MemoryCopy(&m_buffer[j], pbData, i);
	...
	if ((uLen - i) != 0)
		CRT::MemoryCopy(&m_buffer[j], &pbData[i], uLen - i);
}
...
#ifdef SHA1_WIPE_VARIABLES
	CRT::MemorySet(m_buffer, 0, 64);
	CRT::MemorySet(m_state, 0, 20);
	CRT::MemorySet(m_count, 0, 8);
	CRT::MemorySet(pbFinalCount, 0, 8);
	Transform(m_state, m_buffer);
#endif
}
...
bool CSHA1::GetHash(UINT_8* pbDest20) const
{
	...
	CRT::MemoryCopy(pbDest20, m_digest, 20);
	return true;
}

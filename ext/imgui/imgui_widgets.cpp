...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
    ...
    if (ret && it_anim->second.opened_combo)
    {
        ImGuiWindow* popup_window = GetCurrentWindow();
        if (popup_window)
        {
            BringWindowToFocusFront(popup_window);
    ...
    it_anim->second.hovered = IsWindowHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup | ImGuiHoveredFlags_AllowWhenBlockedByActiveItem);

    if (multi && it_anim->second.hovered && g.IO.MouseClicked[0])
        it_anim->second.opened_combo = false;
...
void ImGui::MultiCombo(const char* label, std::vector<bool>& variable, const char* labels[], int count)
{
    if (variable.size() < count)
        variable.resize(count, false);

    std::string preview = "None";

    for (auto i = 0, j = 0; i < count && i < variable.size(); i++)
    {
        if (variable[i])
        {
            if (j)
    ...
    {
        for (auto i = 0; i < count && i < variable.size(); i++)
        {
            if (ImGui::Selectable(labels[i], variable[i]))

...
namespace ImGui
{
    ...
    IMGUI_API ImU32         GetColorU32(const ImVec4& col, float alpha);
    // Cursor / Layout
    ...
    IMGUI_API void          Pickerbox(const char* label, bool* v, float col[3], ImGuiColorEditFlags flags = 0);
    ...
    bool BeginCombo(const char* label, const char* preview_value, float val, bool multi = false, ImGuiComboFlags flags = 0);
    IMGUI_API void          EndCombo(); // only call EndCombo() if BeginCombo() returns true!
    IMGUI_API void          MultiCombo(const char* label, std::vector<bool>& variable, const char* labels[], int count);
